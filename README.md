...
**划分脚本**：`split_dataset.py`（位于数据集仓库）

| 维度 | 规则 |
|------|------|
| **个体筛选** | 按时间跨度筛选（≥3年 / ≥5年 / ≥7年） |
| **划分方法** | 每个个体的图片按时间从旧到新排序，**前 70% → 训练集，后 30% → 测试集** |
| **训练过滤** | 通过 `min_samples_per_identity` 过滤个体（5 = 107个体，15 = 72个体） |

```
个体 t001（20 张图片，时间跨度 2010-2018）
  ↓ 按日期排序
[2010, 2011, 2011, 2012, ..., 2016, 2017, 2018]
  ↓ 7:3 划分
训练集: [2010~2015] 14张  |  测试集: [2016~2018] 6张
```

**这种划分方式的意义**：
- 训练集看到的是**旧照片**，测试集是**新照片**
- 模拟真实场景：用历史数据训练，识别未来拍摄的个体
- 要求模型学习**时间鲁棒的特征**，而非简单记忆外观
...
| **2. 距离三元组** | CE + Batch-Hard Triplet | 200 (从 80 开始) | **69.69%** | **+4.92%** |
| **3. 时间 APN** | CE + Temporal-APN Triplet | 200 (从 80 开始) | **70.21%** | **+5.44%** ✅ |

**关键结论**：
- 距离三元组使 Acc_direct 从 64.77% 提升至 **69.69%**（+4.92%）
- 时间 APN 比距离三元组高 **+0.52%**（70.21% vs 69.69%），优势缩小
- 107 个体上两种 Triplet 差距缩小，说明个体越多、样本越稀疏，APN 的边际效益递减
