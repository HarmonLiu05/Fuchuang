"""
基于文件夹划分的数据集训练脚本
直接读取 data_CZoo_split/train 和 data_CZoo_split/test
"""
import os
import sys
import argparse
...
def main():
    ...
    parser.add_argument('--config', default='configs/config_split.yaml')
    ...
    num_identities = len(train_dataset.classes)
    print(f"身份数量: {num_identities}")
    print(f"训练集: {len(train_dataset)} 样本")
    print(f"测试集: {len(test_dataset)} 样本")
    print(f"身份列表: {train_dataset.classes}")
    ...
    test_loader = DataLoader(
        ...
        shuffle=False, num_workers=config['data'].get('num_workers', 0),
